...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-white text-zinc-900 font-sans pb-20">
      ...
      {/* CABEÇALHO AZUL COM LOGO VITTA.PNG */}
      <nav className="w-full bg-[#0071bb] py-4 px-6 shadow-lg flex justify-between items-center sticky top-0 z-50 animate-in fade-in slide-in-from-top duration-700">
        <div className="flex items-center">
          {/* LOGO VITTA.PNG NO LUGAR DO TEXTO */}
      ...
      {/* SEÇÃO PERFIL */}
      <header className="py-12 px-6 flex flex-col items-center animate-in fade-in zoom-in duration-1000 delay-200">
      ...
      {/* VITRINE DE IMÓVEIS */}
      <section className="max-w-xl mx-auto px-4 mb-16 animate-in fade-in slide-in-from-bottom duration-1000 delay-300">
            ...
            >
              {imovelVitta.images.map((img, i) => (
                <div key={i} className="flex-none w-full h-full snap-center bg-zinc-50">
                  <img src={img} className="w-full h-full object-cover" alt="Vitta Residencial" />
                </div>
              ))}
            </div>
      ...
      {/* CTA WHATSAPP */}
      <section className="flex flex-col items-center px-6 animate-in fade-in duration-1000 delay-500">
        <a 
          href="https://wa.me/5516994633370"
          target="_blank"
          className="w-full max-w-sm p-6 bg-[#0071bb] hover:bg-zinc-900 text-white rounded-2xl transition-all duration-500 flex items-center justify-center gap-3 shadow-xl shadow-blue-100 group"
        >
